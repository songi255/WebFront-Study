const key = "bb9695bea84517c49ce8f0060050ff17";
...
function deleteMemo(){
  document.addEventListener("click", e => { // 어느 영역이든 클릭가능
...
async function getWeather(latitude, longitude){
  if (latitude && longitude){
    return await axios.get(`http://api.openweathermap.org/data/2.5/forecast?lat=${latitude}&lon=${longitude}&appid=${key}`);
  }
  const data = await axios.get(`http://api.openweathermap.org/data/2.5/forecast?q=Seoul&appid=${key}`);
  return await axios.get(`http://api.openweathermap.org/data/2.5/forecast?lat=${data.latitude}&lon=${data.longitude}&appid=${key}`); // ??
}
